...
const ReactNativeQA = () => {
  ...
  React.useEffect(() => {
    GenericService.get(
      `page=${pageNumber}&pagesize=10&order=desc&sort=hot&tagged=react_native&filter=default&site=stackoverflow&run=true`,
      (err: any, data: any) => {
        if (err) {
          Alert.alert(err.toString());
        } else {
          if (pageNumber > 1) {            if (data?.has_more) {
  ...
  return (
    <View style={[{flex: 1}]}>
      <Text style={[{fontSize: 25}]}>React Native Q/A</Text>
      <FlatList
        data={questionsArray}
